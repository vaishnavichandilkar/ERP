...
const EditCategoryModal = ({ isOpen, onClose, data, onSuccess, onShowToast }) => {
    ...
    const handleSave = async () => {
        if (!categoryName.trim()) {
            toast.error(isSubCategory ? t('modules:sub_category_name_required') : t('modules:category_name_required'));
            return;
        }
        ...
        try {
            if (isSubCategory) {
                if (isPromotingToCategory) {
                    ...
                    onShowToast && onShowToast(t('modules:sub_category_promoted_successfully', 'Sub Category moved to Main Category successfully'));
                } else {
            ...
            <div className="relative bg-white rounded-[20px] shadow-[0_20px_50px_rgba(0,0,0,0.15)] w-full max-w-[440px] overflow-hidden transform transition-all duration-300 ease-in-out animate-in zoom-in-95">
                ...
                <div className="flex items-center justify-between px-8 py-5 border-b border-[#04200f] bg-emerald-900 rounded-t-[20px]">
                    <h2 className="text-[18px] font-bold text-white tracking-tight">
                        {isSubCategory ? t('modules:edit_sub_category', 'Edit Sub Category') : t('modules:edit_category')}
                    </h2>
                    <button
                        onClick={onClose}
                ...
                <div className="p-8 space-y-6">
                    ...
                    <div className="space-y-2">
                        ...
                        <input
                            ...
                            disabled={isPromotingToCategory}
                            placeholder={isSubCategory ? t('modules:enter_sub_category_name') : t('modules:enter_category_name')}
                            className={`w-full h-[46px] border border-[#E5E7EB] rounded-[10px] px-4 text-[14px] font-medium outline-none transition-all placeholder:text-gray-400 ${isPromotingToCategory ? 'bg-gray-50 text-gray-400' : 'focus:border-[#073318] focus:ring-4 focus:ring-[#073318]/5'}`}
                        />
                    </div>
                    ...
                    {isSubCategory && (
                        <>
                            ...
                            <div className={`space-y-2 relative transition-all duration-300 ${isPromotingToCategory ? 'opacity-30 pointer-events-none grayscale' : ''}`} ref={parentDropdownRef}>
                                ...
                                {isParentDropdownOpen && (
                                    <div className="absolute top-[calc(100%+4px)] left-0 w-full bg-white border border-[#E5E7EB] rounded-[12px] shadow-xl z-[110] py-2 max-h-[180px] overflow-y-auto animate-in fade-in slide-in-from-top-2 duration-200">
                    ...
                    <div className="flex items-center justify-center gap-3 pt-2">
                        <button
                            onClick={handleSave}
                            disabled={isLoading}
                            className={`flex-1 h-[48px] rounded-[12px] text-[15px] font-bold transition-all flex items-center justify-center gap-2 shadow-md ${isPromotingToCategory ? 'bg-amber-600 hover:bg-amber-700 shadow-amber-600/20' : 'bg-[#073318] hover:bg-[#04200f] shadow-[#073318]/20'} text-white hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed`}
                        >
